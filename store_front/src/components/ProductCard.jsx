  ...
  return (
    <div className="card product-card h-100">
      ...
      <div className="card-body d-flex flex-column">
        ...
        <p className="text-secondary small flex-grow-1">{product.mtg_card?.oracle_text?.slice(0, 100) || product.description?.slice(0, 100) || 'Sin descripción disponible.'}</p>
        <p className="price-highlight mb-1">CLP ${product.price_clp}</p>
        <small className={`mb-3 ${stock > 0 ? 'text-success' : 'text-danger'}`}>Stock: {stock}</small>
