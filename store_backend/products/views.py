...
class PurchaseOrderViewSet(viewsets.ModelViewSet):
    ...
    def recalculate(self, request, pk=None):
        po = self.get_object()
        po = recalculate_purchase_order(po)
        po.refresh_from_db()
        return Response({
            "subtotal_clp": po.subtotal_clp,
            "total_extra_costs_clp": po.total_extra_costs_clp,
            "grand_total_clp": po.grand_total_clp,
            "real_total_clp": po.real_total_clp,
            "items": PurchaseOrderItemSerializer(po.items.all(), many=True).data,
        })
